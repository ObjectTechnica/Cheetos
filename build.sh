...
BuildRoot: %{_tmppath}/Based on the examples found on GitHub, here's an updated version of the script that uses `mock` to build the RPM package:
...
# Prepare the PHP configuration
./configure --prefix=/usr --with-config-file-path=/etc --enable-fpm --with-fpm-user=www-data --with-fpm-group=www-data --disable-short-tags --enable-sockets --enable-sysvsem --enable-sysvshm --enable-pcntl --with-openssl --with-pcre-regex --with
